...
def update_mapping(
    ...
    m = ()
    cat_list = list(category_description)
    for desc in mapping:
        cat_desc = cat_list.pop(0)
        final_desc = desc
        if desc.key == "":
            ...
            for key in [
                ...
                if v := getattr(desc, key):
                    attr_val = getattr(desc, key)
                    if attr_val:
                        attr_val.append(v)
                    else:
                        attr_val = v
                    setattr(cat_desc, key, attr_val)
...
class TuyaBLELight(TuyaBLEEntity, LightEntity):
    ...
    def __init__(
        ...
        if not self._attr_supported_color_modes and (
...
async def async_setup_entry(
    ...
    entities = [
        TuyaBLELight(hass, data.coordinator, data.device, data.product, desc)
        for desc in descs
    ]
