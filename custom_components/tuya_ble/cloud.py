    ...
    async def get_device_credentials(
        ...
        if not force_update and self._has_credentials(self._data):
            ...
            if self._has_login(self._data):
                ...
                for key, cache_item_val in _cache.items():
                    if cache_item_val.credentials.get(address) is not None:
                        cache_key = key
                        break
            if cache_key:
                item = _cache.get(cache_key)

            if (item is None or force_update) and self._is_login_success(
