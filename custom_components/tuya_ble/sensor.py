...
def is_co2_alarm_enabled(self: TuyaBLESensor, _product: TuyaBLEProductInfo) -> bool:
    result: bool = True
            ...
            **{
                key: [
                    ...
                    TuyaBLESensorMapping(
                        dp_id=12,
                        description=SensorEntityDescription(
                            key="unlock_fingerprint", icon="mdi:fingerprint"
                        ),
                    ),
                ]
    ...
    "szjqr": TuyaBLECategorySensorMapping(
        products={
            ...
            **{
                ...
                for key in ["blliqpsj", "ndvkgsrm", "yiihr7zh", "riecov42", "neq16kgd"]
            },
            **{
                key: [TuyaBLEBatteryMapping(dp_id=12)]
                for key in [
        ...
        if self._mapping.getter is not None:
            ...
            if datapoint:
                if datapoint.type == TuyaBLEDataPointType.DT_ENUM:
                    ...
                    if self._mapping.icons is not None and (
                        datapoint.value >= 0
...
async def async_setup_entry(
    ...
    entities: list[TuyaBLESensor] = [
        TuyaBLESensor(hass, data.coordinator, data.device, data.product, rssi_mapping)
    ]
    entities.extend(
        TuyaBLESensor(hass, data.coordinator, data.device, data.product, mapping)
