...
class TuyaBLEDeviceFunction:
    ...
    def __setattr__(self, name: str, value: str | dict | list | None) -> None:
        ...
        if name == "values" and isinstance(value, str) and (v := json.loads(value)):
            value = v
...
class TuyaBLEDevice:
    ...
    def _create_task(self, coro: asyncio.coroutines) -> asyncio.Task:
        """Create a tracked background task to prevent GC before completion."""
    ...
    def _disconnect(self) -> None:
        ...
        self._create_task(self._execute_timed_disconnect())
    ...
    async def _send_packets_locked(self, packets: list[bytes]) -> None:
        ...
        try:
            ...
            if self._is_paired:
                self._create_task(self._resend_packets(packets))
            else:
                self._create_task(self._reconnect())
    ...
    def _handle_command_or_response(
        ...
        match code:
            ...
            case TuyaBLECode.FUN_RECEIVE_TIME1_REQ:
                ...
                self._create_task(self._send_response(code, data, seq_num))
            ...
            case TuyaBLECode.FUN_RECEIVE_DP:
                ...
                self._create_task(self._send_response(code, bytes(0), seq_num))
    ...
    def _parse_input(self) -> None:
        ...
        if raw_length > data_end_pos:
            ...
            (data_crc,) = unpack(
                ">H",
                raw[data_end_pos : data_end_pos + 2],
            )
